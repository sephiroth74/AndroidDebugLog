...
class PostMethodVisitor(
    ...
    private fun printMethodEnd(opcode: Int) {
        ...
        super.visitLdcInsn(methodData.name)                    // methodName (String)
    ...
    private fun printMethodStart() {
        if (methodData.debugEnter) {
            ...
            super.visitLdcInsn(methodData.name)                            // [2] methodName (String)
        ...
        if (methodData.debugExit) {
            logger.lifecycle("[$tagName] $className:${methodData.name} -> adding currentTimeMillis variable")
