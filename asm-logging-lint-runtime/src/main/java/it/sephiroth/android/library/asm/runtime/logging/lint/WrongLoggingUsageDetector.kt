...
class WrongLoggingUsageDetector : Detector(), UastScanner {
    ...
    override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
        ...
        if ("tag" == methodName && evaluator.isMemberInClass(method, "it.sephiroth.android.library.asm.runtime.logging.Trunk")) {
        ...
        if (evaluator.isMemberInClass(method, "android.util.Log")) {
            context.report(
                Incident(
                    ...
                    message = "Using 'Log' instead of 'Trunk'",
                    fix = quickFixIssueLog(node)
                )
            )
            return
        } else if (isTimberLogMethod(method, evaluator)) {
            context.report(
                Incident(
                    issue = ISSUE_TIMBER,
                    scope = node,
                    location = context.getLocation(node),
                    message = "Using 'Timber' instead of 'Trunk'",
                    fix = quickFixIssueTimber(node)
                )
            )
            return
        }
        // Handles Trunk.X(..) where X in (v|d|i|w|e|wtf).
        if (isTrunkLogMethod(method, evaluator)) {
    ...
    private fun isTrunkLogMethod(method: PsiMethod, evaluator: JavaEvaluator): Boolean {
        return evaluator.isMemberInClass(method, "it.sephiroth.android.library.asm.runtime.logging.Trunk")
    }
    ...
    private fun isTimberLogMethod(method: PsiMethod, evaluator: JavaEvaluator): Boolean {
        return evaluator.isMemberInClass(method, "timber.log.Timber")
                ...
                || evaluator.isMemberInClass(method, "timber.log.Timber.Forest")
    ...
    private fun checkNestedStringFormat(context: JavaContext, call: UCallExpression) {
        ...
        while (true) {
            ...
            if (current == null || current is UMethod) {
                // Reached AST root or code block node; String.format not inside Trunk.X(..).
                return
            }
            if (current.isMethodCall()) {
                val psiMethod = (current as UCallExpression).resolve()
                if (psiMethod != null &&
                    Pattern.matches(TRUNK_TREE_LOG_METHOD_REGEXP, psiMethod.name)
                    && isTrunkLogMethod(psiMethod, context.evaluator)
                ) {
                    context.report(
                        Incident(
                            issue = ISSUE_FORMAT,
                            ...
                            message = "Using 'String#format' inside of 'Trunk'",
                ...
                if (argumentType == String::class.java) {
                    ...
                    context.report(
                        Incident(
                            ...
                            message = "Replace String concatenation with Trunk's string formatting",
    ...
    private fun quickFixIssueLog(logCall: UCallExpression): LintFix {
        ...
        // 2nd suggestion drops it (Trunk defaults to calling class name).
        var fixSource2 = "Trunk."
        ...
        return fix().group()
            .add(fix().replace().text(logCallSource).shortenNames().reformat(true).build())
            .add(fix().replace().text(logCallSource).shortenNames().reformat(true).with(fixSource2).build())
            .build()
    }

    private fun quickFixIssueTimber(logCall: UCallExpression): LintFix {
        val arguments = logCall.valueArguments
        val methodName = logCall.methodName
        ...
        when (arguments.size) {
            1 -> {
                val msgOrThrowable = arguments[0]
                fixSource2 += "$methodName(${msgOrThrowable.asSourceString()})"
            }
            2 -> {
                val msg = arguments[0]
                val throwable = arguments[1]
                fixSource2 += "$methodName(${throwable.sourcePsi?.text}, ${msg.asSourceString()})"
            }
            else -> {
                throw IllegalStateException("android.util.Log overloads should have 2 or 3 arguments")
    ...
    companion object {
        ...
        private const val TRUNK_TREE_LOG_METHOD_REGEXP = "(v|d|i|w|e|wtf)"

        val ISSUE_LOG = Issue.create(
            id = "LogNotTrunk",
            briefDescription = "Logging call to Log instead of Trunk",
            explanation = "Since Trunk is included in the project, it is likely that calls to Log should instead be going to Trunk.",
            category = MESSAGES,
            priority = 5,
            severity = WARNING,
            implementation = Implementation(WrongLoggingUsageDetector::class.java, JAVA_FILE_SCOPE)
        )

        val ISSUE_TIMBER = Issue.create(
            id = "TimberNotTrunk",
            briefDescription = "Logging call to Timber instead of Trunk",
            explanation = "Since Trunk is included in the project, it is likely that calls to Timber should instead be going to Trunk.",
            category = MESSAGES,
            priority = 5,
            severity = WARNING,
            implementation = Implementation(WrongLoggingUsageDetector::class.java, JAVA_FILE_SCOPE)
        ...
        val ISSUE_FORMAT = Issue.create(
            id = "StringFormatInTrunk",
            briefDescription = "Logging call with Trunk contains String#format()",
            explanation = "Since Trunk handles String.format automatically, you may not use String#format().",
            category = MESSAGES,
            priority = 5,
            severity = WARNING,
            implementation = Implementation(WrongLoggingUsageDetector::class.java, JAVA_FILE_SCOPE)
        ...
        val ISSUE_THROWABLE = Issue.create(
            ...
            briefDescription = "Exception in Trunk not at the beginning",
            explanation = "In Trunk you have to pass a Throwable at the beginning of the call.",
            category = MESSAGES,
            priority = 5,
            severity = WARNING,
            implementation = Implementation(WrongLoggingUsageDetector::class.java, JAVA_FILE_SCOPE)
        ...
        val ISSUE_BINARY = Issue.create(
            id = "BinaryOperationInTrunk",
            briefDescription = "Use String#format()",
            explanation = "Since Trunk handles String#format() automatically, use this instead of String concatenation.",
            category = MESSAGES,
            priority = 5,
            severity = WARNING,
            implementation = Implementation(WrongLoggingUsageDetector::class.java, JAVA_FILE_SCOPE)
        ...
        val issues = arrayOf(
            ISSUE_LOG, ISSUE_TIMBER, ISSUE_FORMAT, ISSUE_THROWABLE, ISSUE_BINARY, ISSUE_ARG_COUNT, ISSUE_ARG_TYPES,
